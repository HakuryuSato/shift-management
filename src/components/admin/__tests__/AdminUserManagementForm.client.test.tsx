...
const { insertUserAction, deleteUserAction } = require('@/utils/client/serverActionClient');
...
describe('AdminUserManagementForm コンポーネントのテスト', () => {
  ...
  it('ユーザー名を入力して登録が成功すること', async () => {
    useAdminUserManagementFormStore.setState({
      isAdminUserManagementFormVisible: true,
      mode: 'register',
      setUserName: (name: string) => useAdminUserManagementFormStore.setState({ userName: name }),
    });
  ...
  it('削除モードで削除確認モーダルが表示されること', () => {
    ...
    expect(screen.getByText('削除しますがよろしいですか？')).toBeInTheDocument();
  });
