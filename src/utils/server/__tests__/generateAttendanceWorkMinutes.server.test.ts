...
// モック関数にキャスト
const mockedGetHolidays = getHolidays as jest.MockedFunction<typeof getHolidays>;
...
describe('generateAttendanceWorkMinutes', () => {
  ...
  test('8:25から18:00までの通常勤務時間と残業時間を正しく計算できる', async () => {
    ...
    // getHolidays から祝日情報を取得する想定なので、空配列を返す
  ...
  test('12:58から18:01までの通常勤務時間と休憩時間を正しく計算できる', async () => {
    const attendance: Attendance = {
      attendance_id: 2,
      user_id: 1,
      stamp_start_time: '2023-10-02T12:58:00',
  ...
  test('祝日勤務の場合、全時間が残業時間として計算される', async () => {
    const attendance: Attendance = {
      attendance_id: 4,
      user_id: 1,
      stamp_start_time: '2023-10-09T08:25:00', // 祝日
    ...
    mockedGetHolidays.mockResolvedValue([holiday]);
  ...
  test('必須項目が欠けている場合は空のオブジェクトを返す', async () => {
    const incompleteAttendance: Attendance = {
      attendance_id: undefined as any,
      user_id: 1,
      stamp_start_time: '2023-10-02T08:25:00',
      stamp_end_time: '2023-10-02T18:00:00',
      adjusted_start_time: null,
  ...
  test('開始時間が存在しない場合は空のオブジェクトを返す', async () => {
    const incompleteAttendance: Attendance = {
      attendance_id: 5,
      user_id: 1,
      stamp_start_time: undefined as any,
      stamp_end_time: '2023-10-02T18:00:00',
      adjusted_start_time: null,
      adjusted_end_time: null,
      work_minutes: null,
  ...
  test('勤務時間が深夜0時を跨ぐ場合の計算が正しく行われる', async () => {
    const attendance: Attendance = {
      attendance_id: 9,
      user_id: 1,
      stamp_start_time: '2023-10-02T22:30:00',
